    ...
    <script type="text/javascript">
      $(document).ready(function(){
          ...
          $("#status2").change(function(){
            ...
            //alert(datos);
            $.ajax({
              url: "veeeer2.php?id=<?php echo $id; ?>",
              method: "POST",
              data: datos,
    ...
    <form class="formulario2" id="formulario" method="POST">
      ...
      <select class="caja" name ="pro" id = "p">
        <option value="s" selected>- Seleccione -</option>
          <?php
            $query = $link -> query ("SELECT py.id_proyecto, py.nombre FROM `tareas2` as t2 INNER JOIN tareas as ts on ts.id = t2.`id_tarea` INNER JOIN
              proyecto as py on py.id_proyecto = ts.id_proyecto WHERE t2.`id_cuenta`= $id GROUP BY py.id_proyecto");
      ...
      <div>
          <legend>Presupuestos Asignados - Superintendentes</legend>
      <div id="result"></div>
