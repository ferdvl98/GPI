...
$valor = $_POST["caja_valor"]; //jefe de equipo
$id = $_POST["caja_valor2"]; //supervisor
//echo $valor;
//echo $valor;

$uploadfile = $_FILES['uploadfile']['tmp_name'];
/*archivo = $_FILES['uploadfile']['name'];
    $text = pathinfo($uploadfile, PATHINFO_EXTENSION);
            
            if(move_uploaded_file($uploadfile.".".$text, "../archivos/".$archivo)){
                echo "Listo";
                echo $uploadfile." - ".$archivo;
                echo $text;
            }else{
                echo "error  ";
                echo $uploadfile." - ".$archivo;
...
?>
            <script type="text/javascript">
                var id = "<?php echo $id; ?>";
                alert("Hoja de Trabajo Incorrecta -1");
    ...
    for ($i = 0; $i < $tareas4[0]['a']; $i++) {
        ...
        $sql = "UPDATE tareas4 SET hh_real=$nombre WHERE id=$valor";
        ...
        ?>
            <script type="text/javascript">
                var id = "<?php echo $id; ?>";
                alert("Hoja de Trabajo Incorrecta -2");
                window.location = "../tareas/cargar.php?id=" + id;
            </script>
        <?php
        }
    ...
    for ($i = 0; $i < $em[0]['a']; $i++) {
        ...
        $sql2 = "UPDATE equipo_m SET hh_real=$nombre WHERE id=$valor";
        ...
        ?>
            <script type="text/javascript">
                var id = "<?php echo $id; ?>";
                alert("Hoja de Trabajo Incorrecta -3");
                window.location = "../tareas/cargar.php?id=" + id;
            </script>
        <?php
        }
    ...
    $num = $num + 2;
    $nombre = $objExcel->getActiveSheet()->getCell('B' . ($num))->getCalculatedValue();
    if (empty($nombre)) {
        $nombre = "0";
    }
    $sql5 = "SELECT * from analisis where id_equipo = $valor";
    $result5 = $link->query($sql5);

    if ($result5->num_rows > 0) {
        $sql2 = "UPDATE analisis SET ad='$nombre' WHERE id_equipo=$valor";
    } else {
        $sql2 = "INSERT INTO analisis (id_equipo, ad)  VALUES ($valor,'$nombre')";
    }
    ...
    if ($link->query($sql2) === TRUE) {
        ...
        <script type="text/javascript">
            var id = "<?php echo $id; ?>";
            alert("Hoja de Trabajo Incorrecta -4");
            //window.location ="../tareas/cargar.php?id="+id;
        </script>
    <?php
    }
    ...
    if ($result5->num_rows > 0) {
        $sql2 = "UPDATE analisis SET accion='$nombre' WHERE id_equipo=$valor";
    } else {
        $sql2 = "INSERT INTO analisis (id_equipo, accion)  VALUES ($valor,'$nombre')";
    }
    ...
    ?>
        <script type="text/javascript">
            var id = "<?php echo $id; ?>";
            alert("Hoja de Trabajo Incorrecta -5");
            //window.location ="../tareas/cargar.php?id="+id;
        </script>
    <?php
    }
    ...
    //$archivo = $_FILES['uploadfile']['name'];
    $archivo = "../archivos/" . basename($_FILES['uploadfile']['name']);
    while (file_exists($archivo)) {
        $archivo =  "../archivos/" . strval(rand(0, 1000000)) . ".xls";
    }

    if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $archivo)) {
    } else {
        echo "error  ";
    ?>
        <script type="text/javascript">
    ...
    $sql = "INSERT INTO archivos ( `id_super`, `id_jefe`, `ruta`, `nombre`, `fecha`, `vv`)
            VALUES ($id, $valor, '$archivo', '$name','$date', 'checked')";
    ...
    /*
    $datos = array('id_super'=>$id,
                    'id_jefe'=>$valor,
                    'ruta'=>$archivo,
                    'fecha'=>date("Y") . "-" . date("m") . "-" . date("d"));
    $respuesta = $Gestor -> agregarRegistroArchivo($datos);*/
